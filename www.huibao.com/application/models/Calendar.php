...
class CalendarModel extends BaseModel {
    ...
    public $_field = 'fc_id,fc_content, fc_date ,fc_addtime';
    public function __construct()
    {
        parent::__construct('www', 't_finance_calendar', 'fc_id');
    }
    ...
    public function getInfoByDay($day) {
        return $this->_db->select($this->_field)->from($this->_table)->where('fc_date', $day)->fetchOne();
    }
    ...
    private function _setWhereSQL ($where = array()) {
        if(isset($where['day']) && $where['day']){
            $this->_db->where('fc_date', $where['day']);
        }
