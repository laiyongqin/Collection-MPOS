<?php
/**
 * Class representing PhpQuery_Query objects.
 *
 * @author Tobiasz Cudnik <tobiasz.cudnik/gmail.com>
 * @package PhpQuery_Query
 * @method PhpQuery_QueryObject clone() clone()
 * @method PhpQuery_QueryObject empty() empty()
 * @method PhpQuery_QueryObject next() next($selector = null)
 * @method PhpQuery_QueryObject prev() prev($selector = null)
 * @property Int $length
 */
class phpQueryObject
	 ...
	 *
	 * @return PhpQuery_QueryObject|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	 */
...
//			var_dump($documentID->getDocumentID());
		$id = $documentID instanceof self
			...
			: $documentID;
//		var_dump($id);
		if (! isset(PhpQuery_Query::$documents[$id] )) {
//			var_dump(PhpQuery_Query::$documents);
			throw new Exception("Document with ID '{$id}' isn't loaded. Use PhpQuery_Query::newDocument(\$html) or PhpQuery_Query::newDocumentFile(\$file) first.");
		}
		$this->documentID = $id;
		$this->documentWrapper =& PhpQuery_Query::$documents[$id];
...
//		$this->toRoot();
	 ...
	 * @param PhpQuery_QueryObject $var
	 * @return PhpQuery_QueryObject|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	 */
	...
	public function documentFragment($state = null) {
		if ($state) {
			PhpQuery_Query::$documents[$this->getDocumentID()]['documentFragment'] = $state;
			return $this;
		}
...
//		return $this->newInstance(array($this->root));
	 ...
	 * <code>
	 * $myDocumentId;
	 * PhpQuery_Query::newDocument('<div/>')
	 *     ->getDocumentIDRef($myDocumentId)
	 *     ->find('div')->...
	 * </code>
	 *
	 * @param unknown_type $domId
	 * @see PhpQuery_Query::newDocument
	 * @see PhpQuery_Query::newDocumentFile
	 * @return PhpQuery_QueryObject|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	 */
	...
	public function getDocument() {
		return PhpQuery_Query::getDocument($this->getDocumentID());
	}
	/**
	 *
	 * @return DOMDocument
	...
	public function unloadDocument() {
		PhpQuery_Query::unloadDocuments($this->getDocumentID());
	}
	...
	public function serialize() {
		return PhpQuery_Query::param($this->serializeArray());
	}
	/**
	 * Enter description here...
	 *
...
//		$source->dumpDie();
	...
	protected function debug($in) {
		...
		print_r($in);
		// file debug
//		file_put_contents(dirname(__FILE__).'/PhpQuery_Query.log', print_r($in, true)."\n", FILE_APPEND);
...
//			print_r(array_slice(debug_backtrace(), 3));
	...
	protected function isChar($char) {
		return extension_loaded('mbstring') && PhpQuery_Query::$mbstringSupport
			? mb_eregi('\w', $char)
			: preg_match('@\w@', $char);
	}
	/**
...
//		$specialCharsMapping = array('/' => '>');
		...
		// http://code.google.com/p/PhpQuery_Query/issues/detail?id=76
		$_query = array();
		for ($i=0; $i<$strlen; $i++)
...
//				$i = $i+2;
		...
		foreach($args as $callback) {
			if (is_array($return))
				foreach($return as $k => $v)
					$return[$k] = PhpQuery_Query::callbackRun($callback, array($v));
			else
				$return = PhpQuery_Query::callbackRun($callback, array($return));
		}
		return $return;
	}
	/**
	...
	public function newInstance($newStack = null) {
		...
		$new = $class != 'PhpQuery_Query'
			? new $class($this, $this->getDocumentID())
			: new PhpQuery_QueryObject($this->getDocumentID());
		$new->previous = $this;
		if (is_null($newStack)) {
			...
			$new->elements = PhpQuery_Query::pq($newStack, $this->getDocumentID())->stack();
		} else {
...
//		var_dump($this->elements[0]->nodeType);
		...
		foreach($this->stack(array(1, 9, 13)) as $k => $stackNode) {
			...
			foreach($nodes as $node) {
				$matched = false;
				if ( $compare) {
					PhpQuery_Query::$debug ?
						$this->debug("Found: ".$this->whois( $node ).", comparing with {$compare}()")
						: null;
					$PhpQuery_QueryDebug = PhpQuery_Query::$debug;
					PhpQuery_Query::$debug = false;
					// TODO ??? use PhpQuery_Query::callbackRun()
					if (call_user_func_array(array($this, $compare), array($selector, $node)))
						$matched = true;
					PhpQuery_Query::$debug = $PhpQuery_QueryDebug;
				} else {
					$matched = true;
				}
				if ( $matched) {
	...
	public function find($selectors, $context = null, $noHistory = false) {
		...
		// TODO combine code below with PhpQuery_Query::pq() context guessing code
		//   as generic function
		if ($context) {
		...
		foreach($queries as $selector) {
			...
			foreach($selector as $s) {
				// TAG
				$isTag = extension_loaded('mbstring') && PhpQuery_Query::$mbstringSupport
					? mb_ereg_match('^[\w|\||-]+$', $s) || $s == '*'
					: preg_match('@^[\w|\||-]+$@', $s) || $s == '*';
				if ($isTag) {
					if ($this->isXML()) {
				...
				// ERRORS
					...
					PhpQuery_Query::debug("Unrecognized token '$s'");
				}
	...
	protected function pseudoClasses($class) {
		...
		switch($class) {
			...
			case 'submit':
			case 'reset':
				$this->elements = PhpQuery_Query::merge(
						...
						create_function('$node, $param',
							...
							if ($a > 0) {
								return ($index-$b)%$a == 0
									? $node
									: null;
								PhpQuery_Query::debug($a."*".floor($index/$a)."+$b-1 == ".($a*floor($index/$a)+$b-1)." ?= $prevs");
...
//									: null;
	...
	public function is($selector, $nodes = null) {
		PhpQuery_Query::debug(array("Is:", $selector));
		if (! $selector)
			return false;
		$oldStack = $this->elements;
		$returnArray = false;
	...
	public function filterCallback($callback, $_skipHistory = false) {
		...
		foreach($this->elements as $index => $node) {
			$result = PhpQuery_Query::callbackRun($callback, array($index, $node));
						...
						// ATTRS
							...
							if (mb_strpos($attr, '=')) {
								...
								if ($attr == 'nodeType') {
									...
									$val = extension_loaded('mbstring') && PhpQuery_Query::$mbstringSupport
										? quotemeta(trim($val, '"\''))
										: preg_quote(trim($val, '"\''), '@');
									...
									// cut last character
									$attr = substr($attr, 0, -1);
									$isMatch = extension_loaded('mbstring') && PhpQuery_Query::$mbstringSupport
	 ...
	 * @return PhpQuery_Query|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	...
	public function load($url, $data = null, $callback = null) {
		...
		if (mb_strpos($url, ' ') !== false) {
			$matches = null;
			if (extension_loaded('mbstring') && PhpQuery_Query::$mbstringSupport)
				mb_ereg('^([^ ]+) (.*)$', $url, $matches);
			else
	...
	public function __loadSuccess($html) {
		if ($this->_loadSelector) {
			$html = PhpQuery_Query::newDocument($html)->find($this->_loadSelector);
			unset($this->_loadSelector);
		}
		foreach($this->stack(1) as $node) {
			PhpQuery_Query::pq($node, $this->getDocumentID())
				->markup($html);
		}
	}
	/**
	...
	public function trigger($type, $data = array()) {
		foreach($this->elements as $node)
			PhpQuery_QueryEvents::trigger($this->getDocumentID(), $type, $data, $node);
		return $this;
	}
	/**
	...
	public function bind($type, $data, $callback = null) {
		...
		foreach($this->elements as $node)
			PhpQuery_QueryEvents::add($this->getDocumentID(), $node, $type, $data, $callback);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function unbind($type = null, $callback = null) {
		foreach($this->elements as $node)
			PhpQuery_QueryEvents::remove($this->getDocumentID(), $node, $type, $callback);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function wrapAll($wrapper) {
		if (! $this->length())
			return $this;
		return PhpQuery_Query::pq($wrapper, $this->getDocumentID())
			->clone()
   ...
   */
	...
	public function wrap($wrapper) {
		foreach($this->stack() as $node)
			PhpQuery_Query::pq($node, $this->getDocumentID())->wrapAll($wrapper);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function wrapPHP($codeBefore, $codeAfter) {
		foreach($this->stack() as $node)
			PhpQuery_Query::pq($node, $this->getDocumentID())->wrapAllPHP($codeBefore, $codeAfter);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function wrapInner($wrapper) {
		foreach($this->stack() as $node)
			PhpQuery_Query::pq($node, $this->getDocumentID())->contents()->wrapAll($wrapper);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function wrapInnerPHP($codeBefore, $codeAfter) {
		foreach($this->stack(1) as $node)
			PhpQuery_Query::pq($node, $this->getDocumentID())->contents()
	...
	public function contents() {
		$stack = array();
		foreach($this->stack(1) as $el) {
			// FIXME (fixed) http://code.google.com/p/PhpQuery_Query/issues/detail?id=56
...
//		$this->previous->XPath = $this->XPath;
	...
	public function replaceWithPHP($code) {
		return $this->replaceWith(PhpQuery_Query::php($code));
	}
	/**
	 * Enter description here...
	 *
	 * @param String|PhpQuery_Query $content
	...
	public function replaceAll($selector) {
		foreach(PhpQuery_Query::pq($selector, $this->getDocumentID()) as $node)
			PhpQuery_Query::pq($node, $this->getDocumentID())
				->after($this->_clone())
				->remove();
		return $this;
	...
	public function remove($selector = null) {
		...
		foreach($loop as $node) {
			...
			PhpQuery_QueryEvents::trigger($this->getDocumentID(),
	 ...
	 * @return string|PhpQuery_Query|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	...
	public function htmlOuter($callback1 = null, $callback2 = null, $callback3 = null) {
		...
		$args = func_get_args();
		foreach($args as $callback) {
			$markup = PhpQuery_Query::callbackRun($callback, array($markup));
		}
		return $markup;
	}
	/**
	...
	public function markupPHP($code = null) {
		return isset($code)
			? $this->markup(PhpQuery_Query::php($code))
			: PhpQuery_Query::markupToPHP($this->markup());
	}
	/**
	 * Enter description here...
	 * 
	...
	public function markupOuterPHP() {
		return PhpQuery_Query::markupToPHP($this->markupOuter());
	}
	/**
	 * Enter description here...
	 *
		...
		switch(gettype($target)) {
			case 'string':
				...
				if ($to) {
					...
					if (PhpQuery_Query::isMarkup($target)) {
...
//							$loop = $this->find('body > *')->elements;
		...
		PhpQuery_Query::debug("From ".count($insertFrom)."; To ".count($insertTo)." nodes");
...
//						);
	...
	public function index($subject) {
		$index = -1;
		$subject = $subject instanceof PhpQuery_QueryObject
			? $subject->elements[0]
			: $subject;
...
//			return array();
	...
	public function text($text = null, $callback1 = null, $callback2 = null, $callback3 = null) {
		...
		foreach($this->elements as $node) {
			...
			if (count($this->elements) > 1 && $text)
				$text .= "\n";
			foreach($args as $callback) {
				$text = PhpQuery_Query::callbackRun($callback, array($text));
			}
			$return .= $text;
		}
		return $return;
	...
	public function plugin($class, $file = null) {
		PhpQuery_Query::plugin($class, $file);
		return $this;
	}
	/**
	...
	public function __call($method, $args) {
		...
		if (isset(PhpQuery_Query::$extendMethods[$method])) {
			array_unshift($args, $this);
			return PhpQuery_Query::callbackRun(
				PhpQuery_Query::$extendMethods[$method], $args
			);
		} else if (isset(PhpQuery_Query::$pluginsMethods[$method])) {
			array_unshift($args, $this);
			$class = PhpQuery_Query::$pluginsMethods[$method];
			$realClass = "PhpQuery_QueryObjectPlugin_$class";
	...
	public function not($selector = null) {
		if (is_string($selector))
			PhpQuery_Query::debug(array('not', $selector));
		else
			PhpQuery_Query::debug('not');
		$stack = array();
...
//			}
	 ...
	 * Enter description here...
	 *
	 * @param string|PhpQuery_QueryObject
	 * @return PhpQuery_QueryObject|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	 */
	public function add($selector = null) {
		...
		$stack = array();
		$this->elementsBackup = $this->elements;
		$found = PhpQuery_Query::pq($selector, $this->getDocumentID());
	...
	public function attrPHP($attr, $code) {
		if (! is_null($code)) {
			...
			// http://code.google.com/p/PhpQuery_Query/issues/detail?id=17
			...
			foreach($this->stack(1) as $node) {
				...
				if (is_array($val) && in_array($node->attr('type'), array('checkbox', 'radio'))) {
					...
					foreach($node['option']->stack(1) as $option) {
						...
						// http://code.google.com/p/PhpQuery_Query/issues/detail?id=96
...
//								$selected = true;
	...
	public function each($callback, $param1 = null, $param2 = null, $param3 = null) {
		...
		foreach($this->elements as $v)
			PhpQuery_Query::callbackRun($callback, array($v), $paramStructure);
		return $this;
	}
	/**
	...
	public function callback($callback, $param1 = null, $param2 = null, $param3 = null) {
		...
		$params[0] = $this;
		PhpQuery_Query::callbackRun($callback, $params);
		return $this;
	}
	/**
	 * Enter description here...
...
//		}
		...
		return $this->newInstance(
			call_user_func_array(array('PhpQuery_Query', 'map'), $params)
//			PhpQuery_Query::map($this->elements, $callback)
		);
	}
	/**
	 * Enter description here...
	...
	public function data($key, $value = null) {
		if (! isset($value)) {
			...
			return PhpQuery_Query::data($this->get(0), $key, $value, $this->getDocumentID());
		} else {
			foreach($this as $node)
				PhpQuery_Query::data($node, $key, $value, $this->getDocumentID());
			return $this;
		}
	}
	/**
	...
	public function removeData($key) {
		foreach($this as $node)
			PhpQuery_Query::removeData($node, $key, $this->getDocumentID());
		return $this;
	}
...
//		PhpQuery_Query::selectDocument($this->getDocumentID());
		$this->elementsBackup = $this->elements;
...
// 			: array();
   ...
   * @access private
	 ...
	 *
	 * @see PhpQuery_QueryObject::_next()
	 * @return PhpQuery_QueryObject|QueryTemplatesSource|QueryTemplatesParse|QueryTemplatesSourceQuery
	 */
...
//			$namespace .= ':';
	...
	public function dump() {
		print 'DUMP #'.(PhpQuery_Query::$dumpCount++).' ';
		$debug = PhpQuery_Query::$debug;
		PhpQuery_Query::$debug = false;
